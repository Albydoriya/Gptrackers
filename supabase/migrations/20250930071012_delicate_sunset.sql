/*
  ...
  1. New Tables
    - `quotes`
      ...
      - Cost breakdown fields (total_bid_items_cost, shipping costs, fees, etc.)
      - `quote_date` (date)
      - `expiry_date` (date)
      - `notes` (text, optional)
      - `created_by` (uuid, foreign key to auth.users)
      - `converted_to_order_id` (uuid, foreign key to orders)
      - Timestamps
    ...
    - `quote_parts`
      ...
      - `quantity` (integer)
      - `unit_price` (numeric)
      - `total_price` (computed column)
      - `is_custom_part` (boolean)
      - `pricing_tier` (text, optional)
  ...
  3. Functions
    ...
    - `calculate_quote_totals()` - Automatically calculates quote totals

  4. Security
    - Enable RLS on all new tables
    - Add policies for role-based access control
    - Users can manage their own quotes, admins/managers can manage all
...
DROP FUNCTION IF EXISTS calculate_quote_totals();
...
DROP TRIGGER IF EXISTS update_quote_totals_on_insert ON quote_parts;
DROP TRIGGER IF EXISTS update_quote_totals_on_update ON quote_parts;
DROP TRIGGER IF EXISTS update_quote_totals_on_delete ON quote_parts;
DROP TRIGGER IF EXISTS recalculate_quote_totals_on_quote_update ON quotes;
