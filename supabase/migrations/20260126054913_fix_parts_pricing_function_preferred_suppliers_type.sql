/*
  # Fix Parts Pricing Function Return Type

  ## Issue
  The `get_parts_with_latest_pricing` function had a type mismatch where 
  `preferred_suppliers` was declared as `text[]` but the actual database 
  column is `uuid[]`, causing "structure of query does not match function result type" error.

  ## Changes
  1. Drop and recreate the function with correct return type
  2. Change `preferred_suppliers` from `text[]` to `uuid[]`
...
DROP FUNCTION IF EXISTS get_parts_with_latest_pricing(text, text, integer, integer, text, text);
