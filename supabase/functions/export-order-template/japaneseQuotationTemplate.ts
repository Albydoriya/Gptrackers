...
export function applyJapaneseQuotationTemplate(
...
function addHeaderSection(
  ...
  // Empty Rows 7-10 (left side spacing)
  ...
  const totalValueCell = worksheet.getCell('H11');
...
function addPartsTable(
  ...
  headers.forEach((header, index) => {
    ...
    // Bilingual header: Japanese on top line, English below
  ...
  data.parts.forEach((part, index) => {
    ...
    row.getCell(1).font = { name: 'Arial', size: 10, color: { argb: colors.black } };
    ...
    // Amount (formula)
    row.getCell(7).value = { formula: `E${dataRow}*F${dataRow}` };
  ...
  // Update the total formula in H11 with correct range
  const totalValueCell = worksheet.getCell('H11');
  totalValueCell.value = { formula: `SUM(G${firstDataRow}:G${dataRow - 1})` };
...
function addFooterSection(
  ...
  worksheet.mergeCells(footerRow, 6, footerRow, 7);
  const totalLabelCell = worksheet.getCell(footerRow, 6);
  ...
  const totalValueCell = worksheet.getCell(footerRow, 8);
  totalValueCell.value = { formula: `SUM(G13:G${lastPartRow})` };
  totalValueCell.numFmt = '¥#,##0';
  totalValueCell.alignment = { vertical: 'middle', horizontal: 'right' };
...
function configurePageSetup(worksheet: ExcelJS.Worksheet, lastPartRow: number): void {
  ...
  worksheet.pageSetup.printArea = `A1:H${lastPartRow + 3}`;
}
