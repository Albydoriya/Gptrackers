...
function addHeaderSection(
  ...
  const contactCell = worksheet.getCell('F10');
  ...
  const phoneCell = worksheet.getCell('F11');
  ...
  const totalOrdersCell = worksheet.getCell('F12');
  totalOrdersCell.value = `Total Orders: ${data.orders.length}`;
  totalOrdersCell.alignment = { vertical: 'middle', horizontal: 'left', indent: 1 };
  totalOrdersCell.font = { size: 10, color: { argb: colors.text } };
  totalOrdersCell.fill = {
    type: 'pattern',
    pattern: 'solid',
    fgColor: { argb: colors.neutral },
  };
...
function addPartsTable(
  ...
  data.orders.forEach((orderData) => {
    orderData.parts.forEach((part) => {
      ...
      row.getCell(2).font = { size: 9, bold: true, color: { argb: colors.primary } };
